...
for (i,feature) in enumerate(data['features']):
    ...
    if all(x in feature['properties'].keys() for x in ['latitude', 'longitude']):
        #if feature['properties']['latitude'] != '' and feature['properties']['latitude'] != '':
            #total_addresses+=1
            #print feature['properties']['blk_lot']
            """
            if len(feature['properties']['blk_lot']) >= 1:
                if feature['properties']['blk_lot'][-1].isalpha():
                    pass
                    #print feature['properties']['blk_lot']
                    #TODO: Are these separate units??
            blk_lot.append(feature['properties']['blk_lot'])
            """
